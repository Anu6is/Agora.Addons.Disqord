...
namespace Agora.Addons.Disqord.Parsers
{
    public class TimeSpanTypeParser : DiscordGuildTypeParser<TimeSpan>
    {
        public override async ValueTask<ITypeParserResult<TimeSpan>> ParseAsync(IDiscordGuildCommandContext context, IParameter parameter, ReadOnlyMemory<char> value)
        {
            ...
            {
                if (Math.Round(duration.TotalSeconds, 0) < TimeSpan.FromSeconds(5).TotalSeconds)
                    return Failure($"The provided time is too short. Minimum duration is 5 seconds");

                if (Math.Round(duration.TotalSeconds, 0) > TimeSpan.FromMinutes(30).TotalSeconds)
                    return Failure($"The provided time is too long. Maximum duration is 30 minutes");
            }
            else
            {
