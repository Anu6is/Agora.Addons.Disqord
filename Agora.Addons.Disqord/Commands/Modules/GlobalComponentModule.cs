...
namespace Agora.Addons.Disqord.Commands
{
    public sealed class GlobalComponentModule : DiscordComponentModuleBase
    {
        ...
        public GlobalComponentModule(IMediator mediator, ICurrentUserService userService, IUserManager userManager, IEmporiaCacheService cache)
        {
            _cache = cache;
            _mediator = mediator;
                ...
                await dm.SendMessageAsync(
                    new LocalMessage()
                            ...
                            .AddComponent(LocalComponent.Row(
                                LocalComponent.LinkButton(Discord.MessageJumpLink(Context.GuildId.Value, showroomId, reference),"View Item"))));
            }
            catch (Exception) { }
            ...
            await Context.Interaction.Response()
                .SendMessageAsync(
                    new LocalInteractionMessageResponse()
                        ...
                        .AddComponent(LocalComponent.Row(LocalComponent.Button($"finalize:{user}:{showroomId}:{reference}","Submit Final Offer").WithStyle(LocalButtonComponentStyle.Success))));
        ...
        public async Task RejectOfferModal(ulong guildId, ulong channelId, ulong messageId, ulong userId, string reason)
        {
            ...
            try
            {
                ...
                await Context.Interaction.Response().SendMessageAsync(new LocalInteractionMessageResponse().WithContent("Offer removed"));
                await Context.Interaction.Followup().ModifyResponseAsync(x => 
                {
                    x.Content = "Offer rejected";
