...
{
    public class MainSettingsView : BaseGuildSettingsView
    {
        public MainSettingsView(GuildSettingsContext context) :
            base(context,
                ...
                new ("Snipe Trigger", "Remaining listing time before a bid will trigger an extension.", (ctx, opts) => new ServerTimeView(ctx, opts)),
                new ("Snipe Extension", "Duration to extend by if a bid is made within the trigger range.", (ctx, opts) => new ServerTimeView(ctx, opts)),
                ...
                new ("Result Logs", "Channel to send the results of completed listings to.", (ctx, opts) => new ServerTimeView(ctx, opts)),
                new ("Audit Logs", "Channel to log all the actions performed on a listed item.", (ctx, opts) => new ServerTimeView(ctx, opts)),
