...
{
    public class GiveawayRoomView : ChannelSelectionView
    {
        ...
        public GiveawayRoomView(GuildSettingsContext context, List<GuildSettingsOption> settingsOptions, List<Showroom> showrooms)
        ...
        public async ValueTask DeleteRoom(ButtonEventArgs e)
        {
            ...
            await mediator.Send(new DeleteShowroomCommand(new EmporiumId(Context.Guild.Id), new ShowroomId(SelectedChannelId), ListingType.Giveaway));

            _showrooms.RemoveAll(x => x.Id.Value == SelectedChannelId && x.ListingType == ListingType.Giveaway.ToString());
        ...
        {
            var room = _showrooms.FirstOrDefault(x => x.Id.Value == SelectedChannelId && x.ListingType == ListingType.Giveaway.ToString());
            ...
            var reply = await Menu.Interactivity.WaitForInteractionAsync(
                e.ChannelId, x => x.Interaction is IModalSubmitInteraction modal && modal.CustomId == response.CustomId,
            ...
            try
            {
                ...
                await mediator.Send(new UpdateBusinessHoursCommand(new EmporiumId(Context.Guild.Id), room.Id, ListingType.Giveaway, room.ActiveHours));
            }
            catch (Exception ex)
            {
        ...
        public async override ValueTask SaveChannelAsync(SelectionEventArgs e)
        {
            ...
            var room = emporium.Showrooms.FirstOrDefault(x => x.Id.Value.Equals(SelectedChannelId) && x.ListingType.Equals(ListingType.Giveaway.ToString()));

            if (room == null)
            {
                var transactionResult = await data.BeginTransactionAsync(async () =>
                {
                    var roomResult = await mediator.Send(new CreateShowroomCommand(new EmporiumId(Context.Guild.Id), new ShowroomId(SelectedChannelId), ListingType.Giveaway));

                    if (!roomResult.IsSuccessful) return roomResult;

                    if (settings.AvailableRooms.Add(ListingType.Giveaway.ToString()))
                    {
